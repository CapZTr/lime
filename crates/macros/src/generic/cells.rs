...
use lime_generic_def::{CellIndex, CellPat, OperandPat, Pats};
...
impl Cells {
    ...
    pub fn new_operand_types(&self, ast: &ast::OperandPats) -> Result<Pats<OperandPat<CellType>>> {
        Ok(Pats(
            ast.0
                .iter()
                .map(|typ| self.new_operand_type(typ))
                .try_collect()?,
        ))
    }

    pub fn new_operand_type(&self, ast: &ast::OperandPat) -> Result<OperandPat<CellType>> {
